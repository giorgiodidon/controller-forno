...
def temperature_monitor_thread():
    ...
    while True:
        ...
        if temp_data['status'] == 'connected':
            ...
            # Calcola cooling rate se sotto 700 gradi
...
def start_logging():
    """Avvia logging e aggiorna stato programma"""
    try:
        data = request.json
        ...
        # Aggiorna program_state globale - letto da /api/status
        program_state['running']          = True
        program_state['name']             = program_name
        program_state['program_name']     = program_name
        program_state['ramps']            = ramps
        program_state['current_ramp']     = 1
        program_state['total_ramps']      = len(ramps)
        program_state['current_setpoint'] = ramps[0]['target'] if ramps else 0
        program_state['start_time']       = time.time() * 1000  # Millisecondi per JavaScript Date()
...
def stop_logging():
    """Ferma logging e resetta stato programma"""
    logger.stop_logging()
    program_state['running']          = False
    program_state['name']             = ''
    program_state['program_name']     = ''
    program_state['ramps']            = []
    program_state['current_ramp']     = 0
    program_state['total_ramps']      = 0
    program_state['current_setpoint'] = 20.0
    program_state['start_time']       = None
    return jsonify({'success': True})
